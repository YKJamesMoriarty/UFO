"""Broader fall-recovery stress test: goes beyond the exact lie_down_init
training mechanism (which only ever applies a +/-90deg flip about world X) to
test genuinely arbitrary 3D fall orientations.

Two sets:
  A) Systematic yaw sweep: the trained +/-90deg X-axis tip, combined with an
     additional yaw rotation about world Z in {0,90,180,270} deg, composed
     onto the source motion frame's own orientation (same convention as
     training code). Tests whether recovery generalizes across heading/yaw,
     which the lie_down_init mechanism itself never varies explicitly.
  B) Fully arbitrary SO(3): uniformly random 3D orientations (not decomposed
     into any axis-aligned flip), fully replacing the source frame's root
     orientation. This is a genuine "any fall pose" test, decoupled from the
     training reset's specific flip convention.

All drops start from z=0.5m (matching training's lie_down_init height).
Joint pose is taken unmodified from one of a few source motion frames
(cycled), matching training's convention (noise_to_initial_level=0).
"""
...
with torch.no_grad():
    ...
    trials = []
    # Set A: systematic yaw sweep on top of trained X-flip convention
    for sign in (1, -1):
    ...
    # Set B: fully arbitrary SO(3), joint source cycling
    for i in range(16):
        trials.append({
            "set": "B_random_so3",
